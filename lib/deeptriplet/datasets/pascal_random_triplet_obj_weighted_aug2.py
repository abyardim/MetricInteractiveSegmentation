...
class PascalDatasetRandomTripletAugmentedWeighted2(data.Dataset):
    ...
    def _generate_triplet(self, lbl, minrange, maxrange):
        ...
        if np.any((lbl_view.reshape(-1) != 255) & (lbl_view.reshape(-1) != 0)):
            ...
            for i, c in enumerate(counts):
                options = np.nonzero(lbl_view.reshape(-1) == classes[i + delta])[0]
