...
class PadZeros(object):
    ...
    def __init__(self, th=513, tw=512, downsample_label=1,
                 label_fill=255, img_fill=(0.485, 0.456, 0.406)):
        self.th = th
        self.tw = tw
    ...
    def __call__(self, sample):
        img = sample['image']
        mask = sample['label']
        img, mask = self._pad_zeros(img, mask)
    ...
    def _pad_zeros(self, img, lbl):
        ...
        thl, twl = (math.ceil(th / self.downsample_label), math.ceil(tw / self.downsample_label))
        ...
        img_padded[:, :, 0] = self.img_fill[0]
        img_padded[:, :, 1] = self.img_fill[1]
        img_padded[:, :, 2] = self.img_fill[2]

        lbl_padded = np.ones((thl, twl), dtype=np.long) * self.label_fill
...
class RandomRotate(object):
    def __init__(self, degree, fill_color_img):
        self.degree = degree
        self.fill_color_img = fill_color_img

    def __call__(self, sample):
        img = sample['image']
        mask = sample['label']
        rotate_degree = random.uniform(-1*self.degree, self.degree)
        img = img.rotate(rotate_degree, Image.BILINEAR, fill_color=self.fill_color_img)
        mask = mask.rotate(rotate_degree, Image.NEAREST, fill_color=255)
